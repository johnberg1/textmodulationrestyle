...
def vis_faces(log_hooks, txt, mismatch_text):
	...
	plt.gcf().text(0.02, 0.54, 'Original: {}'.format(txt[0]), fontsize=11)
	if mismatch_text:
		plt.gcf().text(0.02, 0.49, 'Mismatch: {}'.format(txt[-1]), fontsize=11)
		plt.gcf().text(0.02, 0.01, 'Mismatch: {}'.format(txt[0]), fontsize=11)
	else:
		plt.gcf().text(0.02, 0.49, 'Mismatch: {}'.format(txt[0]), fontsize=11)
		plt.gcf().text(0.02, 0.01, 'Mismatch: {}'.format(txt[1]), fontsize=11)
	plt.gcf().text(0.02, 0.06, 'Original: {}'.format(txt[1]), fontsize=11)
...
def vis_faces_cycle(log_hooks, txt, mismatch_text):
	display_count = len(log_hooks)
	# n_outputs = len(log_hooks[0]['output_face']) if type(log_hooks[0]['output_face']) == list else 1
	n_outputs = 1
	fig = plt.figure(figsize=(6 + (n_outputs * 2), 4 * display_count))
	gs = fig.add_gridspec(display_count, (2 + n_outputs))
	plt.gcf().text(0.02, 0.54, 'Original: {}'.format(txt[0]), fontsize=8)
	if mismatch_text:
	...
	plt.gcf().text(0.02, 0.06, 'Original: {}'.format(txt[1]), fontsize=8)
	for i in range(display_count):
		hooks_dict = log_hooks[i]
		fig.add_subplot(gs[i, 0])
		vis_faces_iterative_cycle(hooks_dict, fig, gs, i)
	plt.tight_layout()
	return fig
...
def vis_faces_iterative_cycle(hooks_dict, fig, gs, i):
	plt.imshow(hooks_dict['input_face'])
	plt.title('Input\nOut Sim={:.2f}'.format(float(hooks_dict['diff_input'])))
	# fig.add_subplot(gs[i, 1])
	# plt.imshow(hooks_dict['target_face'])
	# plt.title('Target\nIn={:.2f}, Out={:.2f}'.format(float(hooks_dict['diff_views']), float(hooks_dict['diff_target'])))
	# for idx, output_idx in enumerate(range(4, 3, -1)):
	output_image, similarity = hooks_dict['output_face'][2]
	fig.add_subplot(gs[i, 1])
	plt.imshow(output_image)
	plt.title('Output {}\n Target Sim={:.2f}'.format(2, float(similarity)))
	recovered_image, _ = hooks_dict['recovered_face'][2]
